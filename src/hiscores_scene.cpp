...
hiscores_scene::hiscores_scene(session_info &sesh, bn::sprite_text_generator& text_generator) : 
    ...
    _text_generator.set_right_alignment();
        ...
        _text_generator.generate(-74, -43 + z*15, _high_scores_table.at(z).name, _text_sprites);   

        _text_generator.set_center_alignment();
        bn::string<3> level;
        bn::ostringstream lv_string_stream(level);
        lv_string_stream << _high_scores_table.at(z).level;

        bn::string<12> score;
        bn::ostringstream score_string_stream(score);
        score_string_stream << _high_scores_table.at(z).score;
...
void hiscores_scene::init_high_scores_table(){
    ...
    _high_scores_table.emplace_back("POOPSMITH", 80, 4000);
    _high_scores_table.emplace_back("STRONGBAD", 75, 3000);
    _high_scores_table.emplace_back("VIDLCTRX1", 60, 1987);
    _high_scores_table.emplace_back("BUBS", 50, 1000);
